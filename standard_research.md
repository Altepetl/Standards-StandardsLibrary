...
3. **New language.** If the directory does not exist:
   ...
   - **Create `<language>/research-tracking.md`**: one table row per research point of the backbone, with columns **Status | Research Point | Details | Start | End** (status empty = not researched, `✅` done, `❌` error, `⚠️` warning; Details holds the problem description, empty if OK).
